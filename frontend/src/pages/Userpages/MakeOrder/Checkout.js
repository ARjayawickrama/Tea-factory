...
export default function Checkout() {
  ...
  const handleInputChange = (e) => {
    const { name, value } = e.target; // Ensure name and value are correctly destructured
    if (name === 'contact') {
        const numbersOnlyRegex = /^[0-9]*$/;
        if (!numbersOnlyRegex.test(value)) {
            setErrors((prevErrors) => ({
                ...prevErrors,
                contact: 'Contact number can only contain numbers.',
            }));
            return; // Prevent updating the state if invalid
        } else {
            setErrors((prevErrors) => ({
                ...prevErrors,
                contact: '', // Clear the error if valid
            }));
        }
    }
    // Check if name field is being updated and validate it
    if (name === 'name') {
  ...
  const validateField = (name, value) => {
    ...
    if (name === "contact") {
      ...
      if (!contactRegex.test(value)) {
        fieldErrors.contact = "Contact must be exactly 10 digits long.";
      } else {
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      await axios.post(
        "http://localhost:5004/Checkout/confirm-order",
        {
          ...orderDetails,
          cartItems,
          userId,
          ...
          >
            ...
            <div>
              <p className="text-lg font-bold">
                Subtotal: Rs.{item.price * item.quantity}.00
              </p>
            </div>
          </div>
        ))}
