  ...
  return (
    <div className="p-4 border rounded-lg shadow-md mb-4">
      <div className="flex items-center mb-2">
        ...
        <p className="ms-1 text-sm font-medium text-gray-500 dark:text-gray-400 ml-2">{averageRating.toFixed(2)}</p>
      ...
      {ratingDistribution.map((count, index) => (
        <div key={index} className="flex items-center mt-4">
          <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{5 - index} star</span>
          <div className="w-2/4 h-5 mx-4 bg-gray-200 rounded dark:bg-gray-700">
            <div className="h-5 bg-yellow-300 rounded" style={{ width: `${(count / totalRatings) * 100}%` }}></div>
          </div>
          <span className="text-sm font-medium text-gray-500 dark:text-gray-400">{totalRatings > 0 ? ((count / totalRatings) * 100).toFixed(0) : 0}%</span>
        </div>
      ))}
    </div>
  );
